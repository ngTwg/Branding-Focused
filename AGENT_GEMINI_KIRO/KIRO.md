# ANTIGRAVITY CORE RULES — QUY TẮC DỰ ÁN

> **Phiên bản:** 6.2.0 - SOLID-STATE  
...
### RULE 6: EXTERNAL SKILL INTEGRATION (Tích hợp Skills Ngoài)

```
KHI cần skill không có trong ngTwg native (250+ skills):
1. KIỂM TRA: antigravity/skills/ trước — ngTwg luôn là PRIMARY
2. NẾU không đủ: Đọc antigravity/external/SICKN33_BRIDGE.md
3. TÌM: Tag mapping trong Section A của SICKN33_BRIDGE.md
4. APPLY: Governance Wrapper (Section B) — BẮT BUỘC
5. LOAD: external/sickn33-skills/skills/[skill-name]/SKILL.md
6. EXECUTE: Với 5 governance checks (credentials, hallucination, naming, error, evidence)
```

**Nguyên tắc bất biến:**
- ngTwg CONSTITUTION.md và GEMINI.md KHÔNG BAO GIỜ bị ghi đè bởi sickn33
- sickn33 là EXTERNAL LAYER — bổ sung, không thay thế
- Xem quyết định conflict tại: `antigravity/external/OVERLAP_RESOLUTION.md`
- Cài đặt skills: `antigravity/scripts/hybrid-install.ps1`
- Kích hoạt bundles: `antigravity/scripts/activate-bundle.ps1`

**Khi nào dùng sickn33:**
- Cần skill chuyên biệt: SEO, Shopify, Stripe, Azure SDK
- Cần official skill: brand-guidelines-anthropic, pdf-official
- Tag không map vào ngTwg native (check Section A của BRIDGE)

**Khi nào KHÔNG dùng sickn33:**
- Có skill ngTwg tốt hơn (xem OVERLAP_RESOLUTION.md — "ngTwg THẮNG")
- Task liên quan CONSTITUTION, loki-mode, systematic-debugging
...
### Version 6.2.0 (2026-03-26) - SOLID-STATE ERA
- **E2E Autonomy:** Triển khai chu trình sửa lỗi khép kín (Autonomous Loop Closure).
- **Hive-Sync:** Tự động hóa đồng bộ hóa sang RPGITHUB với cơ chế tẩy trắng PII chuyên sâu.
- **Loki-mode v6.2.0:** Nâng cấp khả năng chịu lỗi và tự chữa lành của Orchestrator.
- **Nhóm S:** Bổ sung SA/SB/SC — Solid-State E2E, Hive-Mind Sync, Fault Tolerance.
- **RULE 5:** Planning Before Execution — mandatory for tasks >30 min.
- **RULE 6:** External Skill Integration (sickn33 Bridge) — governance framework for 1,324+ external skills.
...
- **Feedback Loop**: Agent Reviewer có quyền reject code của Agent Coder nếu không đạt chuẩn v6.2.0.
...
# NHÓM E: ENTERPRISE GOVERNANCE & OPERATIONS (v6.2.0)
...
# NHÓM G: KỶ NGUYÊN TỰ TRỊ & TIẾN HÓA (v6.2.0)
...
# NHÓM H: AI SAFETY & AGENTIC BOUNDARIES (v6.2.0)
...
# NHÓM J: TRANSCENDENCE TIER & SOLID-STATE ERA (v6.2.0)
...
# NHÓM S: SOLID-STATE & HIVE-SYNC (v6.2.0)
...
- **Rule 1: Scrubbed Mirroring:** Khi đồng bộ sang `RPGITHUB`, bắt buộc sử dụng lớp lọc chuyên sâu để loại bỏ PII, Credentials, và Artifacts cục bộ (`.db`, `.log`, `.env`).
...
- **Rule 3: Cross-Agent Rule Injection:** Tự động đồng bộ `GEMINI.md` vào tất cả agent configs (`CURSOR.md`, `KIRO.md`, `.cursorrules`, `.kiro/steering/`) để duy trì tính nhất quán về "nhận thức" giữa các môi trường làm việc.

## SC: LOKI-MODE v6.2.0 - FAULT TOLERANCE
...
## SD: GIT REMOTE PERSISTENCE (Lưu trữ URL Repository)

- **Rule 1: Auto-Save Git Link:** Khi thực hiện đẩy code lên GitHub (bao gồm cả khi người dùng gửi ảnh thể hiện việc đẩy code/link repository), Agent BẮT BUỘC phải trích xuất và lưu lại URL repository (`link.git`) vào `PROJECT_MAP.md` hoặc metadata dự án.
...
## THAM CHIẾU SOLID-STATE

- Lõi thực thi E2E: `antigravity/orchestrator.py`
