...
agents = [
    "AGENTS", "AIDER", "CLAUDE", "CLINE", "CONTINUE", "COPILOT", "CURSOR",
    "GEMINI_CLAUDE", "GEMINI_CLINE", "GEMINI_COPILOT", "GEMINI_CURSOR",
    "GEMINI_KIRO", "GEMINI_ROO", "KIRO", "ROO"
]

root_path = r"c:\Users\lengo\RPGITHUB"

template = """# {agent_name_upper}.md - QUY TẮC DỰ ÁN
...
-  **SD protocol:** Triển khai cơ chế lưu trữ link Git Remote tự động.
...
#  {agent_name_upper}_EXPANDED.md — MỞ RỘNG TOÀN DIỆN & NÂNG CẤP SÂU
*(Phần mở rộng bổ sung cho {agent_name_upper}.md — merge toàn bộ vào file gốc)*

> **Phiên bản:** 2.0-EXPANDED  
> **Ngày tạo:** 2026-03-24  
> **Bổ sung cho:** {agent_name_upper}.md (tổng hợp)  
...
- **Self-Audit**: Agent định kỳ tự kiểm tra log hành động của chính mình để đảm bảo tuân thủ {agent_name_upper}.md.
...
# ️ NHÓM H: AI SAFETY & AGENTIC BOUNDARIES (v6.2.0)
...
- **Rule 3: Cross-Model Validation:** Dùng Claude code -> {agent_name_upper} review; giảm điểm mù của một mô hình.
...
- **Rule 1: In-Memory Execution Only:** Cấm lưu trữ core modules dạng file vật lý. Sinh code trực tiếp vào RAM (tmpfs), JIT Compile và chạy thẳng.
- **Rule 2: Continuous AST Polymorphism:** Mỗi 24 giờ, tự động refactor đổi tên biến, cấu trúc logic, biến biến đổi bitwise để chống dịch ngược.
...
- **Rule 4: Neural-Network State Sync:** Đồng bộ trạng thái Agent qua mạng nơ-ron cục bộ.
...
- **Ràng buộc Đa nhiệm:** Code liên quan tới IoT/Hardware phải đi qua máy quét \\vulnerability-scanner\\. Bắt buộc giả lập tín hiệu nhiễu (Hardware-in-the-Loop) và LUÔN cài đặt \\Dead-man switch\\ (Nút kích nổ an toàn - tự ngắt điện/kết nối nếu mất tín hiệu quá 5s).

## GD: THE MEDIA & ARTIFACT SYNTHESIZER (Máy Tổng hợp Truyền thông)
- **Ràng buộc Đa nhiệm:** Khi tạo DOCX/PPTX/PDF/SEO, Agent phải gọi \\content-creator\\ để viết kịch kịch bản tâm lý (Storytelling) trước -> sau đó mới dịch thuật sang XML/OOXML. Các file xuất ra luôn được Watermark định danh, tối ưu Meta Tags và kèm nội dung đa phương tiện sinh động.
...
- Trái tim Điều phối: \\ntigravity/scripts/omni_router.py
...
*Bảo tồn trạng thái và đồng bộ hóa đa môi trường.*

## SD: GIT REMOTE PERSISTENCE
- **Rule 1: Auto-Save Git Link:** Khi thực hiện đẩy code lên GitHub, Agent BẮT BUỘC phải lưu lại URL repository (.git) vào PROJECT_MAP.md hoặc metadata của dự án.
- **Rule 2: Automated Retrieval:** Trong các phiên làm việc sau, Agent phải ƯU TIÊN đọc link repository từ PROJECT_MAP.md để tự động cấu hình remote mà không cần hỏi lại User.
...
for agent in agents:
    agent_dir = os.path.join(root_path, f"AGENT_{agent}")
    if not os.path.exists(agent_dir):
        os.makedirs(agent_dir)
    
    file_name = f"{agent}.md"
    file_path = os.path.join(agent_dir, file_name)
    
    content = template.format(agent_name_upper=agent)
