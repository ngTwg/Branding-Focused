...
> **Phiên bản:** 6.4.0 - UNIFIED SYSTEM
> **Nguyên tắc:** Gọn nhẹ, Tối ưu Token, Siêu Hệ Thống ~1,999 Skills
> **Tổng dòng:** ~600 (Đỉnh cao Tự trị & Đồng bộ)
...
**Tại sao quan trọng:**
- Master Router là bộ não điều phối ~1,999 kỹ năng.
...
**Tổng cộng ~1,999 skills trong 1 thư mục duy nhất: antigravity/skills/**

**1. Frontend (72+ skills):** Master Inventory + `react-best-practices/`, `ui-ux-pro-max/`
**2. Backend (75+ skills):** Master Inventory + `api-patterns/`, `typescript-expert/`
**3. Security (27+ skills):** Master Inventory + `cloud-penetration-testing/`, `vulnerability-scanner/`
**4. DevOps (157+ skills):** `cicd-pipelines.md`, `containerization.md`, `observability.md`
**5. Workflows (44+ skills):** Master Inventory + `debug-protocol.md`, `advanced-testing.md`
**6. Data Engineering (12+ skills):** Master Inventory (`clickhouse`, `cdp`)
**7. Deep Tech & Agents (52+ skills):** Master Inventory + `agent-identifier/`, `mcp-builder/`
**8. Specialized (1,553+ skills):** `specialized-master-inventory.md` (Shopify, PDF, Marketing, SEO, Stripe, Azure, Vercel, etc.)
**9. Beyond Horizon (7+ skills):** `exascale-computing.md`, `green-computing.md`

---

### RULE 5: PLANNING BEFORE EXECUTION (Lập kế hoạch trước khi thực hiện)

```
TRƯỚC KHI bắt đầu BẤT KỲ task phức tạp nào (>30 phút):
1. TẠO FILE PLANNING: <TASK_NAME>_EXECUTION_PLAN.md
2. PHÂN TÍCH: Chia task thành phases và subtasks chi tiết
3. ƯỚC LƯỢNG: Thời gian cho mỗi task
4. ĐÁNH DẤU: ⏳ TODO, 🔄 IN PROGRESS, ✅ DONE
5. CẬP NHẬT: Đánh dấu progress khi hoàn thành từng task
```

**Cấu trúc Planning File:**
```markdown
# TASK EXECUTION PLAN
...
**Lợi ích:**
- Rõ ràng về scope và timeline
- Dễ track progress
- Tránh bỏ sót tasks
- User có thể review trước khi bắt đầu
...
### 5. Planning Before Execution (NEW)
Tasks >30 phút PHẢI có planning file with phases, tasks, and progress tracking.

### 6. Test Before Claim Complete
...
### Version 6.4.0 (2026-03-30) - UNIFIED SYSTEM
-  **Merged Skills:** Tổng hợp toàn bộ sickn33 skills (1,324) vào antigravity/skills/
-  **Single Source:** ~1,999 skills trong 1 thư mục duy nhất
-  **Simplified Routing:** Loại bỏ external layer, routing trực tiếp
-  **Unified Governance:** GEMINI.md + KIRO.md áp dụng cho tất cả skills

### Version 6.3.0 (2024-03-26) - SOLID-STATE ERA
-  **E2E Autonomy:** Triển khai chu trình sửa lỗi khép kín (Autonomous Loop Closure).
-  **Hive-Sync:** Tự động hóa đồng bộ hóa sang Global Repository với cơ chế tẩy trắng PII chuyên sâu.
-  **Loki-mode v6.2.0:** Nâng cấp khả năng chịu lỗi và tự chữa lành của Orchestrator.

### Version 6.2.0 (2024-03-24) - SUPER-SYSTEM
...
**Version:** 6.4.0 (Unified System)
**Last Updated:** 2026-03-30
**Total Skills:** ~1,999 (Merged & Optimized)



#  GEMINI_EXPANDED.md — MỞ RỘNG TOÀN DIỆN & NÂNG CẤP SÂU
*(Phần mở rộng bổ sung cho GEMINI.md — merge toàn bộ vào file gốc)*

> **Phiên bản:** 2.0-EXPANDED
> **Ngày tạo:** 2026-03-24
> **Bổ sung cho:** GEMINI.md (tổng hợp)
...
- **Feedback Loop**: Agent Reviewer có quyền reject code của Agent Coder nếu không đạt chuẩn v 6.2.0.
...
- **Self-Audit**: Agent định kỳ tự kiểm tra log hành động của chính mình để đảm bảo tuân thủ GEMINI.md.
...
## EK: DePIN & TOKENIZED PHYSICAL INFATEUCTURE (2026)
...
- **Visual QA Testing:** Sử dụng Screenshot để so sánh Pixel-perfect with thiết kế Figma.
...
- **Rule 3: Cross-Model Validation:** Dùng Claude code -> Gemini review; giảm điểm mù của một mô hình.
...
- **Rule 3: Multi-Hop Reasoning:** Ép Agent tra cứu tài liệu ít nhất 2 chặng trước khi đưa ra giải pháp code cuối khi.
...
- **Rule 1: In-Memory Execution Only:** Cấm lưu trữ core modules dạng file vật lý. Sinh code trực tiếp vào RAM (    mpfs), JIT Compile and chạy thẳng.
...
- **Rule 3: Gaze & Pinch Interaction:** Hỗ trợ điều khiển bằng Ánh mắt (Gaze-tracking) and Chụm ngón tay (Pinch-to-select).
...
- **Hành động:** Loại bỏ UI phức tạp, tối ưu màu mù-tương-thích, và chia nhỏ luồng thông tin (Scaffolding). Cấm thiết thiết kế gây ức chế.
...
- **Ràng buộc Đa nhiệm:** Code liên quan tới IoT/Hardware phải đi qua máy quét \ ulnerability-scanner\. Bắt buộc giả lập tín hiệu nhiễu (Hardware-in-the-Loop) and LUÔN cài đặt \Dead-man switch\ (Nút kích nổ an toàn - tự ngắt điện/kết nối nếu mất tín hiệu quá 5s).
...
- **Ràng buộc Đa nhiệm:** Khi giải bài tập / dạy học, Agent TUYỆT ĐỐI CẤM in đáp án cuối ra ngay (Anti-Spoonfeeding). Phải bẻ gãy vấn đề, đặt câu hỏi gợi mở, sinh biểu đồ biểu diễn tương tác (\d3js\/\canvas\) and xuất \Excel\ động để người học tự tinh chỉnh tham số.

---
##  THAM CHIẾU THE OMNI-ROUTER
- Trái tim Điều phối: \ antigravity/scripts/omni_router.py

# ️ NHÓM S: SOLID-STATE & HIVE-SYNC (v 6.2.0)
*Kỷ nguyên Thể Vững & Hợp nhất: Tự động hóa chu trình sửa lỗi E2E và đồng bộ hóa đa tầng.*

## SA: SOLID-STATE E2E EXECUTION (Chu trình Thực thi Thể Vững)
- **Rule 1: Autonomous Loop Closure:** Hệ thống phải tự hoàn tất chu trình: Phát hiện lỗi (Lint/Test) -> Lập kế hoạch (Plan) -> Sinh Patch -> Áp dụng -> Xác minh (Checker PASS).
- **Rule 2: Stagnation Guard:** Ngắt mạch nếu Agent lặp lại cùng một hành động hoặc sửa file không thay đổi trạng thái lỗi quá 3 lần.
- **Rule 3: Deterministic Fallback:** Khi LLM (Brain) gặp lỗi hoặc ảo giác, hệ thống tự động sử dụng `FailureMemory` và các Heuristic patches đã được kiểm chứng.

## SB: HIVE-MIND SYNCHRONIZATION (Đồng bộ hóa Hive-Mind)
- **Rule 1: Scrubbed Mirroring:** Khi đồng bộ sang `Global Repository`, bắt buộc sử dụng lớp lọc chuyên sâu để loại bỏ PII, Credentials, và Artifacts cục bộ (`.db`, `.log`, `.env`).
- **Rule 2: Project Mapping & State Continuity:** Mọi Repo con phải có `PROJECT_MAP.md` để lưu trữ trạng thái tự động hóa và phân loại (`PUBLIC` vs `PRIVATE`).
- **Rule 3: Cross-Agent Rule Injection:** Tự động đồng bộ `GEMINI.md` vào `CURSOR.md` hoặc `.cursorrules` để duy trì tính nhất quán về "nhận thức" giữa các môi trường làm việc.

## SC: LOKI-MODE v 6.2.0 - FAULT TOLERANCE
- **Rule 1: Resilient Orchestration:** Orchestrator phải cô lập các lệnh Terminal trong Sandbox và tự hồi sinh tiến trình nếu gặp Crash hệ thống.
- **Rule 2: Tracing & Observability:** Mọi bước thực thi phải được ghi lại qua `TracingService` để phân tích độ trễ, tiêu thụ token và hiệu quả sửa lỗi.

## SD: GIT REMOTE PERSISTENCE (Quy tắc Ghi nhớ Repo)
- **Rule 1: Auto-Save Git Link:** Khi thực hiện đẩy code lên GitHub, Agent BẮT BUỘC phải lưu lại URL repository (link .git) vào `PROJECT_MAP.md` hoặc metadata dự án.
- **Rule 2: Automated Retrieval:** AI phải ưu tiên đọc link đã lưu để tự động cấu hình remote trong các phiên làm việc tiếp theo nếu người dùng yêu cầu tương tác với GitHub.

---
### SD: GIT REMOTE PERSISTENCE (AUTO-SAVE & RECOVERY)
*(Trích xuất link.git từ Terminal/Log/Image và tự động cấu hình remote)*

- **Rule 1: Auto-Save Git Link:** Khi tương tác với GitHub (push, clone, remote add), Agent phải trích xuất `link.git` từ Terminal Output hoặc Git commands. Lưu link này vào `PROJECT_MAP.md` hoặc metadata dự án.
- **Rule 2: Screenshot Link Extraction:** Nếu người dùng gửi ảnh chụp màn hình (Screenshot) chứa Terminal log đẩy code lên GitHub, Agent phải sử dụng OCR/Visual Recognition để bóc tách link repository và lưu lại.
- **Rule 3: Autonomous Remote Discovery:** Trước khi thực hiện lệnh Git lần đầu trong phiên mới, Agent phải kiểm tra Metadata/Project Map. If thấy `link.git` đã lưu, Agent tự động thiết lập `git remote add origin [link]` nếu chưa có.
- **Rule 4: PII Masking during Search:** Khi tìm kiếm link.git trong log, Agent phải đảm bảo không lưu Token/Credentials vào file metadata công khai (Chỉ lưu link public/SSH).

---

## ️ THAM CHIẾU SOLID-STATE
- Lõi thực thi E2E: `antigravity/scripts/orchestrator.py`
- Quy trình đồng bộ: `antigravity/skills/workflows/GITHUB_AUTOMATION_SKILL.md`
