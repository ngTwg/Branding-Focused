/**
 * @script verify_shared_modules.js
 * @version 4.2.0
 * @layer verification
 * @protocol unified-protocol-v1
 * @description Validates the integrity and availability of shared DNA modules (.shared).
 */
...
function logError(error, context) {
    ...
    const entry = `
## [${timestamp}] - Verify-Shared-Modules Script Failure

- **Type**: Runtime
- **Severity**: High
- **File**: \`.agent/scripts/verification/verify_shared_modules.js\`
- **Agent**: DNA Integrity Inspector
- **Root Cause**: ${context}
- **Error Message**: 
  \`\`\`
  ${error.message || error}
  \`\`\`
- **Fix Applied**: N/A
- **Prevention**: Ensure .shared directory structure matches DNA v4.2 spec
...
try {
    ...
    // Note: Since this script is simple, we wrap the whole thing or individual async calls.
} catch (err) {
    logError(err, "General script failure");
}
