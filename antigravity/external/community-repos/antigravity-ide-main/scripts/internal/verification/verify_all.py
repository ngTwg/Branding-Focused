...
@description Full Verification Suite - Runs complete validation including security, performance, and E2E.
"""
...
def log_error(error, context):
    error_log_path = Path("ERRORS.md")
    timestamp = datetime.now().strftime('%Y-%m-%d %H:%M')
    entry = f"""
## [{timestamp}] - Verify-All Script Failure

- **Type**: Runtime
- **Severity**: High
- **File**: \`.agent/scripts/verification/verify_all.py\`
- **Agent**: Senior Principal Quality Auditor
- **Root Cause**: {context}
- **Error Message**: 
  \`\`\`
  {str(error)}
  \`\`\`
- **Fix Applied**: N/A - System alert generated
- **Prevention**: Ensure all dependency scripts in .agent/skills are available
...
if __name__ == "__main__":
    try:
        ...
        print(f"❌ Critical Verification Suite Error: {e}")
        log_error(e, "Uncaught exception in main verification loop")
        sys.exit(1)
